...
resource "alicloud_ecs_disk" "ecs_disk" {
  ...
  disk_name         = "${var.short_name}-${var.role}-${format(var.count_format, count.index + 1)}-data-disk"
  size              = var.ecs_data_disk_size
  category          = var.ecs_data_disk_category
  count             = var.number
  tags = {
...
resource "alicloud_ecs_disk_attachment" "ecs_disk_att" {
  disk_id       = alicloud_ecs_disk.ecs_disk.*.id[count.index]
